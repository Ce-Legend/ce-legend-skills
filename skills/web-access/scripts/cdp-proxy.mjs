#!/usr/bin/env node
// CDP Proxy - 通过 HTTP API 操控本机 Chrome
// 适用于需要用户 Chrome 登录态或更接近日常浏览器环境的场景
// Node.js 22+ 可直接使用原生 WebSocket
...
async function connect() {
  ...
  if (!chromePort) {
    ...
    if (!discovered) {
      throw new Error(
        'Chrome 远程调试尚不可用。请先在 Chrome 打开 chrome://inspect/#remote-debugging 并允许当前浏览器实例的远程调试，或使用 --remote-debugging-port 启动 Chrome。'
      );
    }
    ...
    if (pathname === '/targets') {
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          ...
          '/eval?target=': 'POST body=JS 表达式 - 执行 JS',
          '/click?target=': 'POST body=CSS 选择器 - JS 点击元素',
          '/clickAt?target=': 'POST body=CSS 选择器 - 真实鼠标点击',
          '/setFiles?target=': 'POST body=JSON - 给 file input 设置本地文件',
